...
const Navbar = () => {
  ...
  // Pull userProfile from your AuthContext via useAuth
  const { hasRole, logout, loading, userProfile } = useAuth();
    ...
    <nav className="navbar-wrapper">
          ...
          {visibleLinks.map((link, index) =>
            link.children && link.children.length > 0 ? (
              <Dropdown key={index} className="nav-link fw-semibold text-dark">
                <Dropdown.Toggle variant="link" className="nav-link fw-semibold text-dark">
                  {t(link.translationKey)}
                </Dropdown.Toggle>
                <Dropdown.Menu className="shadow">
                  {link.children.map((child) => (
                    <Dropdown.Item as={Link} key={child.path} to={child.path}>
              ...
              <NavLink key={link.path} to={link.path} className={({ isActive }) => `nav-link fw-semibold ${isActive ? "text-primary" : "text-dark"}`}>
                {t(link.translationKey)}
              </NavLink>
            )
          )}

          <button className={`lang-btn ${isJapanese ? "active" : ""}`} onClick={toggleLanguage}>
            {isJapanese ? "日本語" : "ENG"}
          </button>
      ...
      <div className={`side-panel ${menuOpen ? "open" : ""}`}>
        ...
        {visibleLinks.map((link) => (
          <NavLink key={`${link.name}-${link.path}`} to={link.path} className="nav-link mb-3" onClick={() => setMenuOpen(false)}>
            {t(link.translationKey)}
          </NavLink>
        ))}
